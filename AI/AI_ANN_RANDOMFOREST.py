...
Original file is located at
    https://colab.research.google.com/drive/1sn0hUH9dAkdhCPgV6gVd6ukX0G4BYQHy
"""
...
file_path = "harmonics_data_instantaneousAD.mat"  # Remplace par le bon chemin
...
# Charger le fichier MAT
file_path = "harmonics_data_nofault_instantaneousC.mat"  # Remplace par le bon chemin
data = scipy.io.loadmat(file_path)
...
for i, (te, speed) in enumerate(unique_conditions.values):
    ...
    print("Healthy Machines:")
    print(df_healthy_case)
    print("Faulty Machines:")
...
for te, speed in unique_conditions.values:
    ...
    print(f"Nombre de points suspects après ajout pour Torque: {te}, Speed: {speed} = {len(df_suspects)}")

    # Affichage des distances et caractéristiques dans une seule table
    print(f"\033[1;34mDistances et caractéristiques pour Torque: {te}, Speed: {speed}\033[0m")
...
# Création des sous-plots pour chaque condition unique
fig, axes = plt.subplots(2, 2, figsize=(12, 10))
axes = axes.flatten()
...
for i, (te, speed) in enumerate(unique_conditions.values):
    ...
    df_healthy_case = df_nofault_filtered[(df_nofault_filtered["Te"] == te) & (df_nofault_filtered["speed_i"] == speed)]
    df_faulty_case = df_bruit_filtered[(df_bruit_filtered["Te"] == te) & (df_bruit_filtered["speed_i"] == speed)]
    df_suspect_case = df_suspects[(df_suspects["Te"] == te) & (df_suspects["speed_i"] == speed)] if not df_suspects.empty else pd.DataFrame()
...
# Supprimer les points suspects du dataset
df_bruit_filtered = df_bruit_filtered[~df_bruit_filtered.index.isin(df_suspects.index)]

# Affichage du dataset mis à jour pour chaque condition unique
for te, speed in unique_conditions.values:
    df_case = df_bruit_filtered[(df_bruit_filtered["Te"] == te) & (df_bruit_filtered["speed_i"] == speed)]
...
file_bruit = "new_harmonics_stats.mat"  # Remplace par le bon chemin
file_nofault = "new_nofault_harmonics_stats.mat"  # Remplace par le bon chemin
file_instantaneous = "harmonics_data_instantaneousAD.mat"  # Fichier contenant les valeurs instantanées
...
results_bruit = data_bruit['results']
results_nofault = data_nofault['results']
results_dataset = data_instantaneous['AD']

# Convertir les matrices en DataFrames pandas
df_bruit = pd.DataFrame(results_bruit)
df_nofault = pd.DataFrame(results_nofault)
df_instantaneous = pd.DataFrame(results_dataset, columns=['Te', 'Nf', 'speed_i', 'Ia', 'Iq_2nd_inst', 'Iq_4th_inst'])
...
df_instantaneous["severity"] = severity_fault_dataset
...
if df_suspects.empty:
    ...
    df_filtered_instantaneous = df_instantaneous.merge(df_suspects[columns_to_match], on=columns_to_match, how='left', indicator=True)
    ...
    print(f"Nombre de lignes supprimées : {len(df_instantaneous) - len(df_filtered_instantaneous)}")
...
for index, row in df_instantaneous.iterrows():
...
file_bruit = "new_harmonics_stats.mat"  # Remplace par le bon chemin
file_nofault = "new_nofault_harmonics_stats.mat"  # Remplace par le bon chemin
file_nofault_instantaneous = "harmonics_data_nofault_instantaneousB.mat"  # Fichier contenant les valeurs instantanées sans faute
...
results_bruit = data_bruit['results']
results_nofault = data_nofault['results']
results_nofault_dataset = data_nofault_instantaneous['B']
...
for index, row in df_nofault_instantaneous.iterrows():
    te, speed = row["Te"], row["speed_i"]
    matching_rows = df_nofault_filtered[(df_nofault_filtered["Te"] == te) &
...
for i, (te, speed) in enumerate(unique_conditions.values):
    ...
    # Filtrer les données pour cette condition spécifique
    df_healthy_case = df_nofault_filtered[(df_nofault_filtered["Te"] == te) & (df_nofault_filtered["speed_i"] == speed)]
    df_faulty_case = df_faulty_filtered[(df_faulty_filtered["Te"] == te) & (df_faulty_filtered["speed_i"] == speed)]
...
file_bruit = "new_harmonics_stats.mat"  # Remplace par le bon chemin
file_nofault = "new_nofault_harmonics_stats.mat"  # Remplace par le bon chemin
file_nofault_instantaneous = "harmonics_data_nofault_instantaneousC.mat"  # Fichier contenant les valeurs instantanées sans faute
...
# Sauvegarde du dataset filtré après cette nouvelle étape
filtered_nofault_file_path = "harmonics_data_nofault_refiltered_D.mat"
scipy.io.savemat(filtered_nofault_file_path, {'dataset_nofault_refiltered': filtered_nofault_df.to_numpy()})
...
file_healthy = "harmonics_data_nofault_refiltered_D.mat"
...
for idx, (te, speed) in enumerate(unique_conditions.values):
    ...
    plot_tree(clf.estimators_[0],
              ...
              filled=True,
              ax=ax)
    ax.set_title(f"Decision Tree (Torque={te} Nm, Speed={speed} RPM)", fontsize=20)
...
for idx, (te, speed) in enumerate(unique_conditions.values):
    ...
    plot_tree(clf.estimators_[0],
              ...
              class_names=['Healthy', 'Faulty'],
              filled=True,
              fontsize=8,  # Augmentation de la taille de la police dans les cases
              ax=ax)
    ax.set_title(f"Decision Tree (Torque={te} Nm, Speed={speed} RPM)", fontsize=16)
    plt.show()
...
for idx, (te, speed) in enumerate(unique_conditions.values):
    ...
    plot_tree(clf.estimators_[0],
              ...
              class_names=['Healthy', 'Faulty'],
              filled=True,
              impurity=False,  # Enlever l'affichage de Gini
              fontsize=8,  # Augmentation de la taille de la police dans les cases
              ax=ax)
    ax.set_title(f"Decision Tree (Torque={te} Nm, Speed={speed} RPM)", fontsize=16)
    plt.show()
...
# 🎨 Affichage des matrices de confusion avec pourcentage par ligne
fig, axes = plt.subplots(2, 2, figsize=(12, 10))
axes = axes.flatten()
...
for idx, ((te, speed), results) in enumerate(model_performance.items()):
    ...
    for i in range(cm.shape[0]):
        row_total = np.sum(cm[i, :])  # Total de la ligne
        for j in range(cm.shape[1]):
            percentage = cm[i, j] / row_total * 100  # Calcul du pourcentage pour la ligne
            ax.text(j, i, f"{format(percentage, '.2f')}%\n{cm[i, j]}",
                    ha="center", va="center",
                    color="white" if cm[i, j] > row_total / 2 else "black",
...
# 🎨 Affichage des matrices de confusion avec pourcentage uniquement
fig, axes = plt.subplots(2, 2, figsize=(12, 10))
axes = axes.flatten()
...
fig_tree, axes_tree = plt.subplots(2, 2, figsize=(20, 15))
...
# Pour stocker les matrices de confusion
conf_matrices = {}

for te, speed in unique_conditions.values:
    print(f"\nTraining ANN for Torque = {te}, Speed = {speed}")

    df_case = df_combined[(df_combined['Te'] == te) & (df_combined['speed_i'] == speed)]
    if df_case.empty:
        continue

    X = df_case[['Iq_2nd_inst', 'Iq_4th_inst']].values
    y = df_case['label'].values
