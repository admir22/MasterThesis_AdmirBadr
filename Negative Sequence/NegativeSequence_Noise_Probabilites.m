...
for i = 1:length(Te_values)
    ...
    Rf = Rf_values_OL(i);  % Rounding Rf to 2 decimals
    ...
    results = [results; {Te, speed_i, Nf, sprintf('%.2f', ia), sprintf('%.2f', Rf), sprintf('%.2f%%', moyenne), sprintf('%.2f%%', ecarttype), sprintf('%.2f%%', p1)}];
end

% Creating a MATLAB table with `ia` between `Speed` and `Rf`
T = cell2table(results, 'VariableNames', {'Te[Nm]', 'Speed[rpm]', 'Nf', 'if/in', 'Rf', 'Mean[%]', 'Standard Deviation [%]', 'Detection Probability [%]'});

% Writing to an Excel file
writetable(T, 'probanegseq.xlsx');

disp('Results have been saved in probanegseq.xlsx');
